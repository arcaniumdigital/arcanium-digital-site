...
async function enqueueDueMessage(input: Record<string, unknown>): Promise<unknown> {
  return callFunnelInternal("/internal/due-message", input);
}
...
export const prebookingJourney = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    for (const due of dueSteps) {
      await step.sleepUntil(`sleep-${due.id}`, new Date(submittedAt + due.offsetMs));
      await step.run(due.id, () => enqueueDueMessage({
        leadId: data.leadId,
        messageType: due.messageType,
        completeJourney: due.completeJourney,
      }));
    }
...
export const bookingReminderJourney = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if (reminder24h.getTime() > Date.now()) {
      ...
      await step.run("due-reminder-24h", () => enqueueDueMessage({
        leadId: data.leadId,
        bookingUid: data.bookingUid,
        bookingRevision: data.bookingRevision,
        messageType: "BOOKING_REMINDER_24H_V3",
      }));
    }
    ...
    if (reminder3h.getTime() > Date.now() && reminder3h.getTime() < startAt.getTime()) {
      await step.sleepUntil("sleep-reminder-3h", reminder3h);
      await step.run("due-reminder-3h", () => enqueueDueMessage({
        leadId: data.leadId,
        bookingUid: data.bookingUid,
        bookingRevision: data.bookingRevision,
        messageType: exact3h ? "BOOKING_REMINDER_3H_V3" : "BOOKING_REMINDER_EARLY_V3",
      }));
    }
    return { completed: true };
  },
...
export const inngestHeartbeat = inngest.createFunction(
  {
    id: "arcanium-vendor-audit-heartbeat-v4",
    retries: 3,
    triggers: { cron: "*/15 * * * *" },
  },
  async ({ step }) => step.run("record-worker-heartbeat", () => callFunnelInternal("/internal/inngest-heartbeat", {})),
);
